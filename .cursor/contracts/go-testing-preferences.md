...
- **Methodology:** BDD/TDD, red–green–blue. `rules/asterisk-development.mdc`; contracts mock-pre-investigation through mock-wiring.
...
- [x] **Create rule** — Add `rules/go-testing.mdc` with: prefer standard by default; when to consider a framework; test naming (TestXxx); helpers (t.Helper()); fixtures; no test deps in go.mod unless decided.
...
- [x] **Update rule if needed** — After research: refine `rules/go-testing.mdc` with concrete framework names, use-case mapping, and “consider X when Y” so the rule stays accurate. Re-validate (tests still standard unless adoption is decided).
...
- **Ginkgo adopted (hybrid):** Team decision to adopt Ginkgo + Gomega for BDD/integration specs; use stdlib for simple unit tests. Rule updated (`rules/go-testing.mdc`); added `internal/wiring/suite_test.go` and `internal/wiring/run_spec.go`. Not exclusive: hybrid approach.
...
- 2026-02-09 — Contract created; rule `rules/go-testing.mdc` added with standard-by-default, when-to-use-framework, and conventions. Index updated.
...
- 2026-02-09 — Contract run: Researched testify, ginkgo/gomega, go-cmp, quick, httptest. Wrote `notes/go-testing-frameworks-research.mdc`. Updated `rules/go-testing.mdc` with framework table and “consider X when Y”; added go-cmp and httptest to summary. Re-validated: `go test ./...` passes; no new deps. Research tasks complete.
