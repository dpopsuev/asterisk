...
- **Approaches** — 6 behavioral archetypes (Rapid, Methodical, Analytical, Holistic, Adaptive, Precise) with quantified traits governing speed, persistence, convergence, and failure modes.
- **Personas** — 8 named agent identities (4 Thesis + 4 Antithesis), each with a color, approach, court position, and prompt preamble.
- **Circuit DSL** — YAML circuits compiled to executable directed graphs with conditional edges, zones, and loop control.
...
The framework is the engine; the RCA circuit is one car built on it. See the Origami repository for the full design document.
