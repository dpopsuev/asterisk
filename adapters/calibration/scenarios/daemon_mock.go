...
func DaemonMockScenario() *rca.Scenario {
	return &rca.Scenario{
		Name:             "daemon-mock",
...
func daemonMockRCAs() []rca.GroundTruthRCA {
...
func daemonMockSymptoms() []rca.GroundTruthSymptom {
...
func daemonMockCases() []rca.GroundTruthCase {
	return []rca.GroundTruthCase{
		...
		// Triage sees 2 candidate repos → F2 → F3 → F4 (not dup) → F5 → F6.
		{
			...
			ExpectedResolve: &rca.ExpectedResolve{
				SelectedRepos: []rca.ExpectedResolveRepo{
...
func daemonMockWorkspace() rca.WorkspaceConfig {
