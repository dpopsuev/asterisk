...
func ptpMockRCAs() []rca.GroundTruthRCA {
	return []rca.GroundTruthRCA{
		{
			ID:               "R1",
...
func ptpMockSymptoms() []rca.GroundTruthSymptom {
...
func ptpMockCases() []rca.GroundTruthCase {
	return []rca.GroundTruthCase{
		...
		// Triage sees 2 candidate repos → goes to F2 Resolve → F3 Investigate → F4 → F5 → F6.
		{
			...
			ExpectedLoops:   0,
			ExpectedRecall:  &rca.ExpectedRecall{Match: false, Confidence: 0.0},
			...
			ExpectedResolve: &rca.ExpectedResolve{
				SelectedRepos: ptpResolveRepos("linuxptp-daemon-operator"),
			},
			ExpectedInvest: &rca.ExpectedInvest{
			...
			ExpectedCorrelate: &rca.ExpectedCorrelate{
				IsDuplicate: false, Confidence: 0.3,
			},
			ExpectedReview: &rca.ExpectedReview{Decision: "approve"},
		},
		...
		// Recall hit (H1, confidence 0.95) → skip to F5 Review → F6 Report.
		{
			...
			ExpectedLoops:   0,
			ExpectedRecall: &rca.ExpectedRecall{
		...
		// Triage single candidate (H7) → F3 → F4 correlate (H15 dup >= 0.80) → DONE.
		{
			...
			ExpectRecallHit: false,
			ExpectedLoops:   0,
			ExpectedRecall:  &rca.ExpectedRecall{Match: false, Confidence: 0.1},
			ExpectedTriage: &rca.ExpectedTriage{
				SymptomCategory: "product", Severity: "critical",
				DefectTypeHypothesis: "pb001",
		...
		// H7 single repo → F3 → F4 (correlate dup → DONE).
		{
			...
			ExpectCascade:   true,
			ExpectedLoops:   0,
			ExpectedRecall:  &rca.ExpectedRecall{Match: false, Confidence: 0.15},
			ExpectedTriage: &rca.ExpectedTriage{
...
func ptpMockWorkspace() rca.WorkspaceConfig {
	return rca.WorkspaceConfig{
		Repos: []rca.RepoConfig{
			{
...
func ptpResolveRepos(name string) []rca.ExpectedResolveRepo {
	...
	return []rca.ExpectedResolveRepo{{
		Name:   name,
