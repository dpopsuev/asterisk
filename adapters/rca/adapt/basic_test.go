...
func TestBasicAdapter_UnknownCase(t *testing.T) {
	...
	_, err := a.SendPrompt("nonexistent", string(rca.StepF0Recall), "")
	if err == nil {
...
func TestBasicAdapter_Triage_PTPDomain(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			data, err := a.SendPrompt("C1", string(rca.StepF1Triage), "")
			if err != nil {
				t.Fatalf("SendPrompt: %v", err)
			}
			var result rca.TriageResult
...
func TestBasicAdapter_ComponentIdentification(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a.RegisterCase("C1", tt.info)
			data, err := a.SendPrompt("C1", string(rca.StepF3Invest), "")
			if err != nil {
				t.Fatalf("SendPrompt: %v", err)
			}
			var result rca.InvestigateArtifact
			if err := json.Unmarshal(data, &result); err != nil {
				t.Fatalf("unmarshal: %v", err)
			}
...
func TestBasicAdapter_Recall_NoMatch(t *testing.T) {
	...
	data, err := a.SendPrompt("C1", string(rca.StepF0Recall), "")
	if err != nil {
		t.Fatalf("SendPrompt: %v", err)
	}
	var result rca.RecallResult
	if err := json.Unmarshal(data, &result); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if result.Match {
...
func TestBasicAdapter_Recall_SymptomMatch(t *testing.T) {
	...
	fp := rca.ComputeFingerprint("test", "error", "")
	sym := &store.Symptom{
		Name:            "test",
		Fingerprint:     fp,
		ErrorPattern:    "error",
...
func TestBasicAdapter_Correlate_NoExistingRCA(t *testing.T) {
	...
	data, err := a.SendPrompt("C1", string(rca.StepF4Correlate), "")
	if err != nil {
		t.Fatalf("SendPrompt: %v", err)
	}
	var result rca.CorrelateResult
	if err := json.Unmarshal(data, &result); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
...
func TestBasicAdapter_Correlate_EmptyErrorMessage(t *testing.T) {
	...
	// Use v1 SaveRCA since buildCorrelate uses ListRCAs (v1)
	rcaRec := &store.RCA{Title: "something", Description: "description", Status: "open"}
	st.SaveRCA(rcaRec)
...
func TestBasicAdapter_Resolve_UnknownComponent(t *testing.T) {
	...
	data, err := a.SendPrompt("C1", string(rca.StepF2Resolve), "")
	if err != nil {
		t.Fatalf("SendPrompt: %v", err)
	}
	var result rca.ResolveResult
	json.Unmarshal(data, &result)
