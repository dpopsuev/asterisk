...
func (a *LLMAdapter) SendPrompt(caseID string, step string, _ string) (json.RawMessage, error) {
	...
	// Build case dir
	caseDir, err := rca.EnsureCaseDir(a.basePath, a.suiteID, ctx.storeCase.ID)
	if err != nil {
		return nil, fmt.Errorf("llm: ensure case dir: %w", err)
	}

	// Build prompt params and fill template
	params := rca.BuildParams(a.st, ctx.storeCase, ctx.env, a.catalog, ps, caseDir)
	templatePath := rca.TemplatePathForStep(a.promptDir, ps)
	if templatePath == "" {
		return nil, fmt.Errorf("llm: no template for step %s", step)
	}

	prompt, err := rca.FillTemplate(templatePath, params)
	if err != nil {
	...
	artifactFile := filepath.Join(caseDir, rca.ArtifactFilename(ps))
