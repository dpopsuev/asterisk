...
func TestRoutingRecorder_Records(t *testing.T) {
	...
	resp, err := rec.SendPrompt("C1", string(rca.StepF1Triage), "prompt")
	if err != nil {
		t.Fatal(err)
	}
	...
	if e.Step != string(rca.StepF1Triage) {
		t.Errorf("Step = %q, want %s", e.Step, rca.StepF1Triage)
	}
...
func TestRoutingRecorder_ConcurrentSafety(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func(n int) {
			defer wg.Done()
			_, _ = rec.SendPrompt("C1", string(rca.StepF0Recall), "")
		}(i)
	}
	wg.Wait()
...
var (
	_ rca.ModelAdapter = (*RoutingRecorder)(nil)
	_ rca.StoreAware   = (*RoutingRecorder)(nil)
	_ rca.IDMappable   = (*RoutingRecorder)(nil)
)
