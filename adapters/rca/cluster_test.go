...
func TestClusterDedup_ReducesSteps(t *testing.T) {
	results := make([]CalTriageResult, 5)
	for i := 0; i < 5; i++ {
		results[i] = CalTriageResult{
			Index:      i,
...
func TestClusterDedup_WithinCap(t *testing.T) {
	results := make([]CalTriageResult, MaxClusterSize)
	for i := range results {
		results[i] = CalTriageResult{
			Index:      i,
			CaseResult: &CaseResult{CaseID: "C" + string(rune('1'+i))},
...
func phase5aTriageResults() []CalTriageResult {
	type spec struct {
	...
	results := make([]CalTriageResult, len(specs))
	for i, s := range specs {
		repos := []string{}
		if s.repo != "" {
			repos = []string{s.repo}
...
func legacyClusterKey(tr *CalTriageResult) string {
