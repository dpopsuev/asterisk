...
func TestRunAnalysis_BasicAdapter(t *testing.T) {
	...
	report, err := calibrate.RunAnalysis(st, storeCases, suiteID, cfg)
	if err != nil {
...
func TestRunAnalysis_EmptyCases(t *testing.T) {
	...
	report, err := calibrate.RunAnalysis(st, nil, 1, cfg)
	if err != nil {
...
func TestFormatAnalysisReport_Structure(t *testing.T) {
	report := &calibrate.AnalysisReport{
		LaunchName: "test-launch",
		Adapter:    "stub",
		TotalCases: 2,
		CaseResults: []calibrate.AnalysisCaseResult{
			{
				CaseLabel:   "A1",
	...
	output := calibrate.FormatAnalysisReport(report)
	if len(output) == 0 {
